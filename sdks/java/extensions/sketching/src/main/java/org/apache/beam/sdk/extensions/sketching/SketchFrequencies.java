 ...
 *
 * <h2>References</h2>
 * The papers and other useful information about it is available on <a
 * href="https://sites.google.com/site/countminsketch/">this website</a>. <br>
 * The implementation comes from <a
 * href="https://github.com/apache/spark/tree/master/common/sketch">Apache Spark</a>.
 *
 * <h2>Parameters</h2>
 *
 ...
 *
 * <p>These two parameters will determine the size of the Count-min sketch, which is in fact
 * a two-dimensional array with depth and width defined as follows :
 * <ul>
 *   <li>{@code depth = ceil(2 / epsilon)}</li>
 *   <li>{@code width = ceil(-log(1 - confidence) / log(2))}</li>
 * </ul>
 *
 * <p>With the default value, this gives a depth of 200 and a width of 10.
 *
 *  <h2>Examples</h2>
 *
 ...
 *       advanced processing involving the Count-Min {@link Sketch}.
 * </ul>
 *
 * <h3>Example 1: simple default use</h3>
 *
 * <p>The simplest use is simply to call the {@link #globally()} or {@link #perKey()} method
 * in order to compute the sketch.
 *
 * <pre><code>
 ...
 *
 * <p>This example shows how to query the Count-min {@link Sketch} in order to
 * build a KV Pair linking each element to the estimation of its frequency. <br>
 * Here the sketch is used as a {@link ParDo}'s sideInput. <br>
 *
 * <pre><code>
 * {@literal PCollection<MyObject>} pc = ...;
 * {@literal PCollection<CountMinSketch>} countMinSketch = pc.apply(SketchFrequencies
 ...
 * {@literal final Coder<MyObject>} = pc.getCoder();
 ...
 *
 * <p>The CombineFn does the same thing as the transform but it can be used for doing stateful
